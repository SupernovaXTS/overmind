...
export class FeederOverlord extends Overlord {
	...
	constructor(directive: DirectiveFeeder, priority = OverlordPriority.sectorLogi.feeder) {
	...
	private handleFeeder(feeder: Zerg): void {
		...
		if (feeder.carry.energy == 0) {
			...
			if (!target) {
				log.warn(`${this.print}: no energy withdraw target for ${feeder.print} in source colony ${this.parentColony.print}!`);
				return;
			}
