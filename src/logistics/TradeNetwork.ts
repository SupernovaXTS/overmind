...
export class TraderJoe implements ITradeNetwork {
	...
	private buildMarketCache(verbose = false, orderThreshold = 1000): void {
		...
		const allOrders = Game.market.getAllOrders(
			(order) =>
				!myActiveOrderIDs.includes(order.id) && order.amount >= orderThreshold
	...
	private buildMarketHistoryCache(): void {
		...
		for (const resource in historyByResource) {
			...
			const std14 = Math.sqrt(
				_.sum(
					resourceHistory,
					(h) => h.volume * (h.avgPrice - avg14) ** 2 + h.stddevPrice ** 2
	 ...
	 */
	private marginalTransactionPrice(
		order: Order,
		dealerRoomName: string
	): number {
		if (order.roomName) {
			const transferCost =
				Game.market.calcTransactionCost(10000, order.roomName, dealerRoomName) /
				10000;
	 ...
	 */
	private getPriceForBaseIngredients(
		resource: ResourceConstant /*, colony?: Colony*/
	): number {
	...
	private computeCompetitivePrice(
		...
		if (type == ORDER_SELL) {
			...
			if (
				(!Abathur.isBaseMineral(resource) && price < priceForBaseResources) ||
	...
	private maintainOrder(
		...
		if (type == ORDER_SELL) {
			amount = Math.min(
				amount,
				TraderJoe.settings.market.orders.maxBuyOrderAmount
			);
		} else {
			amount = Math.min(
				amount,
				TraderJoe.settings.market.orders.maxSellOrderAmount
			);
		}
	...
	private cleanOrders() {
		const ordersToClean = _.filter(Game.market.orders, (order) => {
			...
			if (
				Game.time - order.created > TraderJoe.settings.market.orders.timeout &&
	...
	private buyDirect(
		...
		if (
			amount < TraderJoe.settings.market.orders.minBuyDirectAmount &&
		...
		const adjustedPrice =
			order.price + this.marginalTransactionPrice(order, terminal.room.name);
	...
	private sellDirect(
		...
		if (
			amount < TraderJoe.settings.market.orders.minSellDirectAmount &&
			!opts.ignoreMinAmounts &&
			!opts.dryRun
		) {
			return NO_ACTION;
...
export class TraderJoeIntershard implements ITradeNetwork {
	 ...
	 * Returns whether an order is yours
	 */
	isOrderMine(order: Order): boolean {
		return Game.rooms[order.roomName!] && Game.rooms[order.roomName!].my;
	}
	...
	priceOf(mineralType: InterShardResourceConstant): number {
		if (this.memory.cache.history[mineralType]) {
			return this.memory.cache.history[mineralType].avg;
		} else {
			return Infinity;
