	...
	private registerRequests() {
		...
		if (
			...
			for (const [resourceType, amount] of this.factory.store.contents) {
	 			...
	 			this.transportRequests.requestOutput(
	 				this.factory,
	 				Priority.Low,
	 				{ resourceType: resourceType as ResourceConstant, amount }
	 			);
			}
		}
		...
		for (const [component, amount] of recipe) {
			...
			this.transportRequests.requestInput(this.factory, Priority.High, {
	 			resourceType: component as ResourceConstant,
				amount: cappedAmount,
			});
		}

		this.debug(() => `contents: ${JSON.stringify(contents)}`);
		...
		for (const [component, amount] of entries(contents)) {
 			...
 			this.transportRequests.requestOutput(
 				this.factory,
 				Priority.Normal,
 				{ resourceType: component as ResourceConstant, amount: amount }
 			);
		}
	}
		...
		if (this.canProduce) {
			...
			if (result === OK) {
				...
				if (!this.memory.stats.totalProduction[product.commodityType]) {
		 			this.memory.stats.totalProduction[product.commodityType as string] =
		 				0;
				}
